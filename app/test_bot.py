...
class TestBot(unittest.TestCase):
    ...
    def test_bot_can_apply_strategies(self):
        ...
        params = self.bot.get_params({'is_bet': False, 'is_insurance': False, 'win_lose': 0, 'hand': hand, 'up_card': 6, 'shoe': Shoe(), 'player': True})
    ...
    def test_bot_can_decide_to_make_a_bet(self):
        ...
        self.shoe.count = 36
        move = self.bot.make_bet(True, False, 0, hand, 1, self.shoe)
    ...
    def test_bot_can_decide_to_get_insurance(self):
        hand = Hand(bet=10).set_cards([{'r': 8, 's': 1}, {'r': 9, 's': 1}])
        self.shoe.count = 36
        move = self.bot.get_insurance(False, True, 0, hand, 1, self.shoe)
    ...
    def test_bot_stands_if_hand_card_total_is_17_or_above(self):
        hand = Hand(bet=10).set_cards([{'r': 8, 's': 1}, {'r': 9, 's': 1}])
        move = self.bot.make_a_move(False, False, 0, hand, 6, self.shoe)
    ...
    def test_bot_hits_if_hand_card_total_is_less_than_17(self):
        ...
        move = self.bot.make_a_move(False, False, 0, hand, 7, self.shoe)
